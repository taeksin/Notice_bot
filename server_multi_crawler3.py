...
def Hansung_scrapeNotices():
    ...
    for tableRow in tableRows:
        ...
        url = Hansung_BASE_URL + \
            href if href.startswith("/") else Hansung_BASE_URL + "/" + href
...
def Computer_scrapeNotices():
    ...
    for a_tag in reversed(a_tags[-10:]):
        ...
        url = Computer_BASE_URL + \
            href if href.startswith("/") else Computer_BASE_URL + "/" + href
        result.append(Notice(id, title, url))
        if result.__len__() == 10:
            break
...
while True:
    ...
    if Hansung_index > 0:
        for i in range(int(Hansung_index)):
            ...
            Hansung_notice_title = Hansung_Result[i].title.replace(
                '\t', '').replace('\n', '')
    ...
    if Computer_index > 0:
        for i in reversed(range(int(Computer_index))):
            ...
            Computer_notice_title = Computer_Result[i].title.replace(
                '\t', '').replace('\n', '')
