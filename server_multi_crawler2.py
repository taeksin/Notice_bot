...
# firebase_admin.initialize_app(cred,{'databaseURL' : 'https://boogienotice-default-rtdb.firebaseio.com/' })
...
while True:
    try:  # 예외 처리 블록 시작
        ...
        print(f'Hansung_std={Hansung_std}')
        f.close()
        ...
        # 파일 열기
        with open('Hansung_IDS.txt', 'r') as file:
        # 각 줄을 읽어서 숫자로 변환
        ...
        # 정렬된 결과를 다시 파일에 쓰기
        with open('Hansung_IDS.txt', 'w') as file:
        ...
        # 파일 열기
        with open('Computer_IDS.txt', 'r') as file:
        # 각 줄을 읽어서 숫자로 변환
            numbers = [int(line.strip()) for line in file]
        ...
        # 정렬된 결과를 다시 파일에 쓰기
        with open('Computer_IDS.txt', 'w') as file:
            for number in numbers:
                file.write(str(number) + '\n')
        ...
        if Computer_index >= 1:
            for i in reversed(range(int(Computer_index))):
                ...
                # Computer_ref.update({Computer_Result[i].id : Computer_Result[i].title}) #해당 변수가 없으면 생성한다.
                # Computer_ref = db.reference("Notice_List/Computer_Notice/ID_URL") #db 위치 지정, 기본 가장 상단을 가르킴
                # Computer_ref.update({Computer_Result[i].id : Computer_Result[i].url}) #해당 변수가 없으면 생성한다.
                # print(" ->이메일 알림  및  DB 저장 완료\n")
        ...
        print("예외 발생:", str(e))
        message = {'content': f'예외 발생: {str(e)} \n 나 오류났어 나좀 고쳐줘'}  # 예외 메시지를 디스코드 웹훅으로 전송
        send_message(message)
        time.sleep(3600)
        #sys.exit(1)  # 예외 발생 시 프로그램 종료
    # server.quit()
